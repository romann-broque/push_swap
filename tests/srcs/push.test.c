...
/*   Updated: 2023/01/29 17:45:34 by rbroque          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	push_test2(const size_t nb)
{
	int			in_nb1[] = {1};
	int			*out_nb1 = NULL;
	int			out_nb2[] = {1};
...
static int	push_test3(const size_t nb)
{
	int			*out_nb1 = NULL;
	int			out_nb2[] = {1, 3};
	size_t		out_size_a = 0;
	...
	stack_a = NULL;
	stack_b = gen_list(out_nb2, out_size_b);
	push(&stack_a, &stack_b);
	ret_val = is_lst_expected(stack_a, out_nb1, out_size_a)
				&& is_lst_expected(stack_b, out_nb2, out_size_b);
	ft_lstclear(&stack_a, NULL);
